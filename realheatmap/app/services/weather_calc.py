from sqlalchemy.orm import Session
from realheatmap.app.database.models import WeatherCalculated
from datetime import date
from typing import Optional
import math

DAILY_WEIGHT_MAP = {
    1: 0.85, 2: 0.85,
    3: {(1, 10): 0.9, (11, 20): 0.95, (21, 31): 1.0},
    4: {(1, 10): 1.0, (11, 20): 0.95, (21, 30): 0.9},
    5: 0.85, 6: 0.8, 7: 0.33, 8: 0.33,
    9: 0.5, 10: 0.61, 11: 0.78, 12: 0.83
}

DWI_THRESHOLDS = [
    (0.0000, 0.1183), (0.1184, 0.1878), (0.1879, 0.2571),
    (0.2572, 0.3320), (0.3321, 0.4089), (0.4090, 0.4932),
    (0.4933, 0.5861), (0.5862, 0.6862), (0.6863, 0.7820),
    (0.7820, 1.0000)
]

def get_daily_weight(target_date: date) -> float:
    month = target_date.month
    day = target_date.day
    rule = DAILY_WEIGHT_MAP.get(month)

    if isinstance(rule, dict):
        for (start, end), weight in rule.items():
            if start <= day <= end:
                return weight
        return 1.0
    return rule or 1.0

def get_dwi_score(adjusted_score: float) -> Optional[int]:
    for i, (low, high) in enumerate(DWI_THRESHOLDS, start=1):
        if low <= adjusted_score <= high:
            return i
    return 10

def calculate_fire_risk_score(db: Session, region: str, target_date: date) -> Optional[float]:
    row = db.query(WeatherCalculated).filter_by(region=region, date=target_date).first()
    if not row:
        print(f"❌ [오류] 기상 계산 데이터 없음 → region='{region}', date={target_date}")
        return None

    weather = {
        "Tmean": row.temperature,
        "Rh": row.humidity,
        "Eh": row.effective_humidity,
        "Wmean": max(row.wind, 0.1)
    }

    try:
        month = target_date.month

        if 1 <= month <= 6:
            risk_score = 1 / (1 + math.exp((2.706
                                + 0.088 * weather["Tmean"]
                                - 0.055 * weather["Rh"]
                                - 0.023 * weather["Eh"]
                                - 0.014 * (weather["Wmean"])) ** -1))
        else:
            risk_score = 1 / (1 + math.exp((1.099
                                + 0.117 * weather["Tmean"]
                                - 0.069 * weather["Rh"]
                                - 0.182 * (weather["Wmean"])) ** -1))
    except OverflowError:
        print(f"⚠️ [경고] OverflowError 발생 → region='{region}', date={target_date} | 기본 score=0 사용")
        risk_score = 0

    daily_weight = get_daily_weight(target_date)
    adjusted_score = risk_score * daily_weight
    dwi_score = get_dwi_score(adjusted_score)

    print(f"✅ [계산 완료] DWI Score={dwi_score} | adjusted={adjusted_score:.4f} | region='{region}', date={target_date}, weight={daily_weight}")
    return dwi_score